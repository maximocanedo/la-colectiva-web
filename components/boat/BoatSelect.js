...
export default class BoatSelect {
    ...
    constructor() {
        const id = BoatSelect.getRandomId("BoatSelect");
        this.getId = () => id;
        this.boats = [];
        this.page = 0;
        this.items = 8;
        ...
        const txt = new TextField({variant: "outlined", label: "Embarcación", icon: "directions_boat", type: "text"});
    ...
    getBoatListItems(arr) {
        const docks = [];
        arr.map(boat => {
            const listItem = new BoatSelect.ListItem(boat.name, boat.mat, boat._id, this.getId(), boat._id);
            docks.push(listItem);
        });
        return docks;
    }
    addItems(arr) {
        const docks = this.getBoatListItems(arr);
        docks.map(dock => {
    ...
    async more() {
        ...
        const source = await fetch("https://colectiva.com.ar:5050/boats/?q="+this.q+"&prefer=-1&itemsPerPage="+this.items+"&p="+page, {
            method: "GET",
            credentials: "include"
        });
        if(source.ok) {
            const data = await source.json();
            this.boats = [ ...this.boats, ...data ];
            this.addItems(data);
        }
    }
    async search() {
    ...
    renderDialog() {
        ...
        md.root.classList.add("boat-select--dialog");
        return {dialog, md};
    }
    render() {
